...
(function() {
  $(document).ajaxComplete(function(event, request) {
    ...
    if (request.getResponseHeader("X-Message-Type").indexOf("danger") !== -1) {
      alert_type = 'alert-danger';
    }
    if (request.getResponseHeader("X-Message-Type").indexOf("keep") !== 0) {
      if (msg) {
        $("#flash_hook").replaceWith("<div id='flash_hook'> <div class='container'> <div class='row'> <div class='alert " + alert_type + "'> <button type='button' class='close' data-dismiss='alert'>&times;</button>" + msg + "</div> </div> </div> </div>");
      }
      if (!msg) {
